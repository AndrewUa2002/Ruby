...
class PersonRepository
  ...
  def self.add_person(first_name = nil, last_name = nil, inn = nil, birth_date = nil)
    if first_name && last_name && inn && birth_date
      new_person = Person.new(first_name, last_name, inn, birth_date)
    else
      new_person = Person.new
    end

    if new_person.is_a?(Person)
      @persons << new_person
    end
    puts new_person
  end
...
if $PROGRAM_NAME == __FILE__
  ...
  PersonRepository.add_person('Oleksandra', 'Vishenko', 'AA2233445566', Date.new(2000, 2, 2)) # Add person with arguments
  PersonRepository.add_person('Roman', 'Realov', 'AB3235337860', Date.new(1995, 5, 12)) # Add person with arguments
